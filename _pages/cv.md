...
- Processed Nanopore Fast5 files (base calling, re-squiggle, signal-level analysis).  
- Extracted methylation site information and performed accurate prediction of methylation sites.  
- Applied Multi-view Model to improve accuracy by over 5% vs m6Anet on HEK293T dataset.  
- Manuscript submitted to *NUCLEIC ACIDS RESEARCH* (Under review).
...
- Processed NASA GeneLab datasets (OSD-402, OSD-403).  
- Conducted QC, clustering, annotation, cell communication, and pseudotime analysis using Seurat and Monocle3.  
- Manuscript submitted to *npj Microgravity* (Under review).
...
- Conducted GO, KEGG, GSEA, GSVA functional enrichment analyses.  
- Applied Random Forest algorithm for transcriptomics of hypoxia vs normoxia groups.  
- Manuscript submitted to *Science Bulletin* (Submitted).
...
- Discovered non-independent modification occurrences and long-range dependencies (>100–1000 bp).  
- Revealed cross-talk between m6A and m5C in HEK293T; biological validation ongoing.  
...
- Led architecture design and technology selection (Vue.js + Spring Boot).  
- Designed and implemented security framework using Apache Shiro (RBAC, session, auth).  
- Developed backend APIs for drug, supplier, inventory, and sales modules.  
- **Open-source:** [GitHub Link](#)

---

### A Fine-tuned Large Model based on DeepSeek for AI-Assisted Interviews  
**Project Leader** | *Mar. 2025 – Jun. 2025*  
**Tech Stack:** Python, PyTorch, QLoRA, Ollama, HTML, Vue 3  

- Built world’s largest Chinese interview dataset (150k+ Q&A pairs), published on Hugging Face.  
- Fine-tuned DeepSeek model with QLoRA (Unsloth) for AI interviewer tasks (questioning, scoring).  
- Developed full-stack web app (Vue 3 + Ollama backend).  
- **Open-source:** [GitHub Link](#)  
